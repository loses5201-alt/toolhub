...
- 截圖美化(screenshot-beauty,category=workshop):替截圖加漸層/純色/透明背景、留白、圓角、陰影,
  做成簡報、社群貼文、作品集、教學文的成品圖。可選 1:1 / 16:9 / 4:3 / 3:4 / 9:16 輸出比例(內容自動
  置中外擴留白,符合各平台版位)、1×/2× 解析度、PNG/JPG。線上同類(Shots.so / Pika 等)常要上傳截圖、
  加浮水印或付費;本工具全程 Canvas 合成、不上傳。支援拖放/檔案選擇/Ctrl+V 貼上;8 組漸層預設;
  圓角用 arcTo 路徑 clip 後 drawImage,陰影先以圓角矩形投影再裁切貼圖;漸層依角度算對角線投影起訖點。
  render(scale) 單一渲染函式同時供即時預覽(1×)與輸出(可 2×),畫布上限 8000px 自動降倍防當機;
  透明背景輸出 JPG 自動鋪白底防變黑。零三方相依(純 Canvas)、不上傳;type-check + 全測試 + build 通過 — 2026-06-18
