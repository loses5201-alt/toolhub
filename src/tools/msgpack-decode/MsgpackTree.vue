...
import type { MsgpackNode } from '@/features/msgpack'

/* MessagePack 結構樹的遞迴節點顯示(陣列用 children,map 用 entries)。 */
defineProps<{ nodes: MsgpackNode[]; depth: number }>()
</script>
    ...
    <li v-for="(node, i) in nodes" :key="i">
      <div class="flex flex-wrap items-baseline gap-x-2 gap-y-0.5" :style="{ paddingLeft: depth * 16 + 'px' }">
        ...
        <span
          ...
          :class="['array', 'map'].includes(node.type) ? 'font-semibold text-brand-700' : 'text-ink-800'"
        >{{ node.value }}</span>
        <span v-if="node.error" class="text-[11px] text-rose-600">⚠️ {{ node.error }}</span>
      </div>
      <MsgpackTree v-if="node.children && node.children.length" :nodes="node.children" :depth="depth + 1" />
      <ul v-if="node.entries && node.entries.length" class="space-y-1.5">
        <li v-for="(ent, j) in node.entries" :key="j">
          ...
          <MsgpackTree v-if="ent.value.children || ent.value.entries" :nodes="[ent.value]" :depth="depth + 2" />
        </li>
      </ul>
    </li>
  </ul>
